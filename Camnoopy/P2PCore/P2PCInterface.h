...
enum
{
    ...
    MESG_TYPE_SET_MOTOR_PRESET_POS = 87,
...
#pragma mark - 获取 布防类型的预置位置
    ...
    MESG_TYPE_RET_ALARM_TYPE_MOTOR_PRESET_POS,//91
...
#pragma mark - 温湿度
    MESG_TYPE_GET_TH_DATA = 92,  //92   获取温湿度数据
    MESG_TYPE_SET_TH_DATA = 93,  //93   设置温湿度
    MESG_TYPE_RET_TH_DATA = 94,	 //94   设置返回值
    
    MESG_TYPE_SET_GPIO_CTL = 95,
    MESG_TYPE_RET_GPIO_CTL, //值：96
    
#pragma mark - 设备重启
    MESG_TYPE_SET_REMOTE_REBOOT = 109,  //值：109
    MESG_TYPE_RET_REMOTE_REBOOT = 110,  //值：110
...
enum
{
    FUCTION_TEST_ID_REPORT_3CID, // 一键报号功能
    FUCTION_TEST_ID_SOS,         // SOS 紧急报警;
    FUCTION_TEST_ID_PIR,         // PIR(按顺序执行,可由 PC 端处理:布防、关闭移动侦测、打开 PIR、 打开喇叭);
    FUCTION_TEST_ID_RESET,       // 恢复出厂;
    FUCTION_TEST_ID_MONTOR,     // 马达自转(开启、停止);
    FUCTION_TEST_ID_LED,        // 指示灯(跑马灯)。
};
...
typedef struct sP2PInitPrm
{
    ...
    void (* vFlagUpdate)(DWORD *pdwFlags );//得到新的系统消息后，调此函数映射回，停止回调
#endif
