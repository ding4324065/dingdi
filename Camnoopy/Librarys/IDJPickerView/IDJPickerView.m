...
//        self.bg=[UIImage imageNamed:@"date_bg.png"];
    ...
    if (!self.picker_wheel_left) {
        self.picker_wheel_left=[UIImage imageNamed:@"time_setting_bg.png"];
    }
    if (!self.picker_wheel_right) {
        self.picker_wheel_right=[UIImage imageNamed:@"time_setting_right.png"];
    }
...
//    [bgImage release];
    ...
    UIImageView *leftImage=[[UIImageView alloc]initWithFrame:CGRectMake(0, 3, 25, self.frame.size.height)];
    leftImage.image=[self.picker_wheel_left resizableImageWithCapInsets:UIEdgeInsetsMake(self.picker_wheel_left.size.height, self.picker_wheel_left.size.width, self.picker_wheel_left.size.height, self.picker_wheel_left.size.width)];
    
    
    UIImageView *rightImage=[[UIImageView alloc]initWithFrame:CGRectMake(self.frame.size.width-5, 3, 10, self.frame.size.height)];
    rightImage.image=[self.picker_wheel_right resizableImageWithCapInsets:UIEdgeInsetsMake(self.picker_wheel_right.size.height, self.picker_wheel_right.size.width, self.picker_wheel_right.size.height, self.picker_wheel_right.size.width)];
    
    [self addSubview:leftImage];
    [self addSubview:rightImage];
    self.pickerLeftView = leftImage;
    [self.pickerLeftView setHidden:self.leftViewIshidden];
    [leftImage release];
    [rightImage release];
}
...
-(void)setFrame:(CGRect)frame{
    ...
    self.selectedCenterView.frame = CGRectMake(10,selectionY,8,rowHeight+1);
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(18, selectionY, self.frame.size.width-30, 1)];
    label.backgroundColor = XBlack;
    UILabel *label2 = [[UILabel alloc]initWithFrame:CGRectMake(18, selectionY+rowHeight, self.frame.size.width-30, 1)];
    label2.backgroundColor = XBlack;
    [self addSubview:label];
    [self addSubview:label2];
    self.line1 = label;
    [self.line1 setHidden:self.lin1Ishidden];
    self.line2 = label2;
    [self.line1 setHidden:self.lin2Ishidden];
...
-(void)setline1Hidden:(BOOL)hidden{
    self.lin1Ishidden = hidden;
    if(self.line1){
        [self.line1 setHidden:hidden];
    }
}

-(void)setline2Hidden:(BOOL)hidden{
    self.lin2Ishidden = hidden;
    if(self.line2){
        [self.line2 setHidden:hidden];
    }
}

-(void)setleftViewHidden:(BOOL)hidden{
    self.leftViewIshidden = hidden;
    if (self.pickerLeftView) {
        [self.pickerLeftView setHidden:hidden];
    }
}
...
- (void)_setSelectionArea {
    ...
    //中间平铺
    UIImageView *selectionCenterView=[[UIImageView alloc]initWithFrame:CGRectMake(10,selectionY,8,rowHeight+1)];
    selectionCenterView.backgroundColor = XBlue;
    
//    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, selectionY, self.frame.size.width, 1)];
//    label.backgroundColor = XBlack;
//    UILabel *label2 = [[UILabel alloc]initWithFrame:CGRectMake(0, selectionY+rowHeight, self.frame.size.width, 1)];
//    label2.backgroundColor = XBlack;
//    [self addSubview:label];
//    [self addSubview:label2];
    self.selectedCenterView = selectionCenterView;
