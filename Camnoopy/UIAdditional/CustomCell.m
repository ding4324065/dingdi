...
-(void)layoutSubviews{
    ...
    if(self.newsDeviceIcon&&self.newsDeviceIcon.length>0){
        if(!self.newsDeviceIconView){
            ...
            UIImage *newDeviceIcon = [UIImage imageNamed:self.newsDeviceIcon];
            newDeviceIconView.image = newDeviceIcon;
            [self.backgroundView addSubview:newDeviceIconView];
            self.newsDeviceIconView = newDeviceIconView;
            [newDeviceIconView release];
        }else{
            self.newsDeviceIconView.frame = CGRectMake(self.textLabelView.frame.origin.x+labelTextWidth+5.0,(BAR_BUTTON_HEIGHT-img_h)/2,img_w, img_h);
            self.newsDeviceIconView.contentMode = UIViewContentModeScaleAspectFit;
            UIImage *newDeviceIcon = [UIImage imageNamed:self.newsDeviceIcon];
            self.newsDeviceIconView.image = newDeviceIcon;
            [self.backgroundView addSubview:self.newsDeviceIconView];
        }
    }
