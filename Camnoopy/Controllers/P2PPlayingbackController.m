...
-(void)onSingleTap{
    ...
    if (self.isShowControllerBar) {
        ...
        [UIView transitionWithView:self.controllerView duration:0.2 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        self.isShowControllerBar = !self.isShowControllerBar;
        [UIView transitionWithView:self.controllerView duration:0.2 options:UIViewAnimationOptionCurveEaseInOut
            animations:^{
...
-(void)P2PPlaybackReject:(NSDictionary *)info{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            usleep(800000);
            dispatch_async(dispatch_get_main_queue(), ^{
                //                [self dismissViewControllerAnimated:YES completion:nil];视频回放挂断问题
                MainContainer *mainController = [AppDelegate sharedDefault].mainController;
                [mainController dismissP2PView];
