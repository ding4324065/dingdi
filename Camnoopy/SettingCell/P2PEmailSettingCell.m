...
#define LEFT_LABEL_WIDTH 115//存储信息cell
...
#pragma mark - tableView图标
        ...
        [leftIconView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(decShieldID)]];
        ...
        [self.leftIconView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(decShieldID)]];
...
-(void)decShieldID{
    if ([self.delegate respondsToSelector:@selector(havetapLeftIconView:androw:)]) {
        [self.delegate havetapLeftIconView:self.section androw:self.row];
    }
}
