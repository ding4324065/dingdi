...
- (void)CreatePickerView{
    timeBroadcastView = [[UIView alloc] initWithFrame:self.frame];
...
//    UIView * middlebule = [[UIView alloc] initWithFrame:CGRectMake(0, rowheight*2+1, timeBroadcastView.frame.size.width/40, rowheight)];
//    [middlebule setBackgroundColor:[UIColor blueColor]];
//    [timeBroadcastView addSubview:middlebule];
...
- (void)createscrollview{
    ...
    self.scrollWidthProportion=[[NSMutableArray alloc]initWithCapacity:array.count];
    for (NSInteger i=0; i<array.count; i++) {
        [_scrollWidthProportion addObject:[NSString stringWithFormat:@"%f", [[array objectAtIndex:i]floatValue]/total]];
    }
    
//设置每个滚轮的个数
    self.numberOfCellsInScroll = [[NSMutableArray alloc] initWithCapacity:array.count];
    self.scrollviews = [[NSMutableArray alloc] initWithCapacity:array.count];
    for (NSInteger i = 0; i<array.count; i++) {
...
- (UIView *)pageAtIndex:(NSInteger)index andScrollView:(MXSCycleScrollView *)scrollView
{
    ...
    for (MXSCycleScrollView * sc in self.scrollviews) {
        if (scrollView.tag == sc.tag) {
            cnt = sc.tag-100;
            l.text = [NSString stringWithFormat:@"%d",index+[[self.valueOfCellsInScroll objectAtIndex:cnt] integerValue]];
        }
    }
