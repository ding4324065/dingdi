...
-(void)initComponent{
    ...
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, width, height-NAVIGATION_BAR_HEIGHT) style:UITableViewStyleGrouped];//UITableViewStyleGrouped,headView随cell一起滚动
...
-(void)defenceCell:(DefenceCell *)defenceCell section:(NSInteger)section row:(NSInteger)row{
    ...
    if (section > 0) {
        if ([switchStatusData[row] intValue] == 0) {//启用
            ...
            [[P2PClient sharedClient] setDefenceSwitchStateWithId:self.contact.contactId password:self.contact.contactPassword switchId:self.lastSetSwitchType alarmCodeId:section-1 alarmCodeIndex:row];
        }else{//禁用
            self.isLoadDefenceSwitch = YES;
            [self.tableView reloadRowsAtIndexPaths:[NSArray arrayWithObjects:[NSIndexPath indexPathForRow:row inSection:section], nil] withRowAnimation:UITableViewRowAnimationNone];
