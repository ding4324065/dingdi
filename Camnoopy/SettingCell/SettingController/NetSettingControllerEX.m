...
@interface NetSettingControllerEX ()

@end

@implementation NetSettingControllerEX
-(void)dealloc{
    [self.names release];
    [self.types release];
        ...
        case RET_SET_NPCSETTINGS_NET_TYPE:
        {
            ...
            if(result==0){
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (result == 45) {
//                        修改失败，可能设备不支持有线连接
                        [self.view makeToast:NSLocalizedString(@"operator_failure_unsupport_wire", nil)];
                    }
                    else if (result == 46) {
//                        修改失败
    ...
    switch (section) {
        case 0:
        {
            if(row==0){
                ...
                emailCell.contentView.backgroundColor = [UIColor whiteColor];
                if(!self.isLoadingNetType){
                ...
                netTypeCell.contentView.backgroundColor = [UIColor whiteColor];
        ...
        case 1:
        {
            if(row==0){
                ...
                wifiCell.contentView.backgroundColor = [UIColor whiteColor];
